...
local Trove = require(Ramka.GetClass("Trove"))
...
--[=[
	...
	```lua
	local Mouse = require(SharedPackages.Input).Mouse
	```
]=]
local Mouse = {}
Mouse.__index = Mouse
...
--[=[
	Similar to :Show and :Hide but uses boolean instead.
]=]
function Mouse:SetVisibleToBoolean(boolean)
	UserInputService.MouseIconEnabled = boolean
	self.mouseVisible = boolean
end

--[=[
	Checks if the mouse cursor is visible.
]=]
function Mouse:IsVisible(): boolean
	return self.mouseVisible
end
