...
local typeMetadata = {
	...
	NumberSequenceKeypoint = {
		...
		fromIntermediate = function(value)
			return NumberSequenceKeypoint.new(value[1], value[2], value[3])
		end,
	},
	...
	NumberSequence = {
		...
		toIntermediate = function(value)
			...
			for _,info in keypoints do
				local t = {"Time","Value","Envelope"}
				for i = 1,3,1 do
					increment += 1
					setKeys[increment] = info[t[i]]
				end
			end
		...
		fromIntermediate = function(value)
			local newKeys = {}
			for i = 1,#value,3 do
				table.insert(newKeys,NumberSequenceKeypoint.new(value[i],value[i+1],value[i+2]))
			end

			return NumberSequence.new{table.unpack(newKeys)}
		end,
	},
...
local springData: {[Instance]: {[string]: any}} = {}
	...
	for instance, state in springStates do
		for propName, spring in state do
			if spring:canSleep() then
				...
				springData[instance]._details.Updated:Fire((instance :: any)[propName])
				springData[instance]._details.Stopped:Fire(spring.rawGoal)
				springData[instance].isMoving = false
			else
				(instance :: any)[propName] = spring:step(dt)
				springData[instance]._details.Updated:Fire((instance :: any)[propName])
				springData[instance].isMoving = true
			end
		end
...
do
	...
	function spr.link(instance)
		if springData[instance] then
			return springData[instance]
		else
			local Began,Stopped,Updated = Instance.new("BindableEvent"),Instance.new("BindableEvent"),Instance.new("BindableEvent")
	...
	function spr.clear(instance,key:table?)
		key = key or springData[instance]
		for i,v in key do
	...
	function spr.target(instance: Instance, dampingRatio: number, frequency: number, properties: {[string]: any})
		...
		--if springData[instance] == nil then
			spr.link(instance)
		--end

		-- if springData[instance] then
		springData[instance]._details.Began:Fire()
		-- end
	...
	function spr.stop(instance: Instance, property: string?)
		...
		springData[instance]._details.Stopped:Fire()
